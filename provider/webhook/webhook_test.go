...
func TestInvalidDomainFilter(t *testing.T) {
	svr := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/" {
			w.Header().Set(endpoint.ContentTypeHeader, endpoint.MediaTypeFormatAndVersion)
			w.WriteHeader(200)
			return
		}
		w.Write([]byte(`[{
...
func TestApplyChangesWithProviderSpecificProperty(t *testing.T) {
	...
	err = p.ApplyChanges(context.TODO(), &endpoint.Changes{
