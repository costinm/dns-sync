...
func NewWebhookProvider(u string) (*WebhookProvider, error) {
	...
	req.Header.Set(acceptHeader, endpoint.MediaTypeFormatAndVersion)
	...
	err = backoff.Retry(func() error {
		...
		if err != nil {
			slog.Warn("Failed to connect to webhook", "url", u, "err",  err)
			return err
		}
	...
	contentType := resp.Header.Get(endpoint.ContentTypeHeader)
	...
	if contentType != endpoint.MediaTypeFormatAndVersion {
		return nil, fmt.Errorf("wrong content type returned from server: %s", contentType)
	}

	slog.Info("DNS provider", "url", u, "filter", string(dfbytes))
...
func (p WebhookProvider) Records(ctx context.Context) ([]*endpoint.Endpoint, error) {
	...
	if err != nil {
		...
		slog.Warn("Failed to create request", "url", u, "err", err.Error())
		return nil, err
	}
	req.Header.Set(acceptHeader, endpoint.MediaTypeFormatAndVersion)
	...
	if err != nil {
		recordsErrorsGauge.Inc()
		slog.Warn("Failed to perform request", "url", u, "err", err.Error())
		return nil, err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		recordsErrorsGauge.Inc()
		slog.Warn("Failed to get records", "url", u, "code", resp.StatusCode)
	...
	if err := json.NewDecoder(resp.Body).Decode(&endpoints); err != nil {
		recordsErrorsGauge.Inc()
		slog.Warn("Failed to decode records", "url", u, "err", err)
		return nil, err
	}
...
func (p WebhookProvider) ApplyChanges(ctx context.Context, changes *endpoint.Changes) error {
	...
	if err := json.NewEncoder(b).Encode(changes); err != nil {
		...
		slog.Warn("Failed to encode changes","err", err)
		return err
	}
	...
	if err != nil {
		applyChangesErrorsGauge.Inc()
		slog.Warn("Failed to create request", "url", u, "err", err)
		return err
	}

	req.Header.Set(endpoint.ContentTypeHeader, endpoint.MediaTypeFormatAndVersion)

	resp, err := p.client.Do(req)
	if err != nil {
		applyChangesErrorsGauge.Inc()
		slog.Warn("Failed to perform request", "url", u, "err", err)
		return err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusNoContent {
		...
		slog.Warn("Failed to apply changes", "url", u, "code",resp.StatusCode, "res", string(body), "retry", isRetryableError(resp.StatusCode))
...
func (p WebhookProvider) AdjustEndpoints(e []*endpoint.Endpoint) ([]*endpoint.Endpoint, error) {
	...
	if err != nil {
		...
		slog.Warn("Failed to join paths", "url", u, "err", err)
		return nil, err
	}
	...
	if err := json.NewEncoder(b).Encode(e); err != nil {
		adjustEndpointsErrorsGauge.Inc()
		slog.Warn("Failed to encode endpoints", "url", u, "err", err)
		return nil, err
	}
	...
	if err != nil {
		adjustEndpointsErrorsGauge.Inc()
		slog.Warn("Failed adjustEndpoints request", "url", u, "err", err)
		return nil, err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		adjustEndpointsErrorsGauge.Inc()
		slog.Warn("Failed adjustEndpoints request", "url", u, "code", resp.StatusCode)
	...
	if err := json.NewDecoder(resp.Body).Decode(&endpoints); err != nil {
		adjustEndpointsErrorsGauge.Inc()
		slog.Warn("Failed adjustEndpoints decode", "url", u, "err", err)
		return nil, err
	}
