...
import "github.com/costinm/dns-sync/endpoint"

// Policy allows to apply different rules to a set of changes.
type Policy interface {
	Apply(changes *endpoint.Changes) *endpoint.Changes
}
...
var Policies = map[string]Policy{
	...
	"upsert": &UpsertOnlyPolicy{},
	"create": &CreateOnlyPolicy{},
}
...
func (p *SyncPolicy) Apply(changes *endpoint.Changes) *endpoint.Changes {
	return changes
}
...
func (p *UpsertOnlyPolicy) Apply(changes *endpoint.Changes) *endpoint.Changes {
...
func (p *CreateOnlyPolicy) Apply(changes *endpoint.Changes) *endpoint.Changes {
	return &endpoint.Changes{
		Create: changes.Create,
	}
}
