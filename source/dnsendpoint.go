...
// This can be used with the ExternalDNS CRD or with any CRD that implements the
// pattern ( includes the same json representation )
...
func NewCRDClientForAPIVersionKind(client kubernetes.Interface, kubeConfig, apiServerURL, apiVersion, kind string) (*rest.RESTClient, *runtime.Scheme, error) {
	...
	// TODO(costin): pass a K8S config, to allow using multi-cluster mode.
