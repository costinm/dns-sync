...
func (tf TargetNetFilter) Match(target string) bool {
	if len(tf.FilterNets) == 0 && len(tf.excludeNets) == 0 {
		return true
	}
	...
	return matchTargetNetFilter(ip, tf.FilterNets, target, true) && !matchTargetNetFilter(ip, tf.excludeNets, target, false)
}
...
func matchTargetNetFilter(ip net.IP, filters []*net.IPNet, target string, emptyval bool) bool {
	if len(filters) == 0 {
		return emptyval
	}
