...
//
// Unlike the external-dns source, which allows any Service to claim FQDNs with
// annotations, this is focused on syncing all Services using a well-defined
// name pattern.
//
// The pattern is similar to K8S standard, but has more information and uses
// a custom suffix.
...
func NewMeshServiceSource(ctx context.Context, name string, p source.ClientGenerator, cfg *endpoint.SourceSpec, config *endpoint.SyncConfig) (endpoint.Source, error) {
	kubeClient, err := p.KubeClient()
	if err != nil {
		return nil, err
	}
	// TODO: rename to avoid confusion and keep the config struct clean
	labelSelector, err := labels.Parse(cfg.Labels)
	if err != nil {
		return nil, err
	}
	...
	if cfg.InternalDomain == "" {
		cfg.InternalDomain = ".svc.mesh.internal"
	}

	// wait for the local cache to be populated.
	if err := source.WaitForCacheSync(context.Background(), informerFactory); err != nil {
		return nil, err
	}
...
func (sc *serviceSource) Endpoints(ctx context.Context) ([]*endpoint.Endpoint, error) {
	...
	for _, svc := range services {
		...
		for _, s := range svcEndpoints {
			s.Labels["ns"] = svc.Namespace
			// TODO: identify the service acccount (secure naming)
			s.Labels["m"] = sc.SyncID
		}
...
func (sc *serviceSource) extractHeadlessEndpoints(svc *v1.Service, hostname string, ttl endpoint.TTL) []*endpoint.Endpoint {
	...
	endpointsType := source.GetEndpointsTypeFromAnnotations(svc.Annotations)

	targetsByHeadlessDomainAndType := make(map[endpoint.EndpointKey]endpoint.Targets)
	if endpointsObject != nil {
		for _, subset := range endpointsObject.Subsets {
			...
			for _, address := range addresses {
				...
				for _, headlessDomain := range headlessDomains {
					targets := source.GetTargetsFromTargetAnnotation(pod.Annotations)
					if len(targets) == 0 {
						if endpointsType == source.EndpointsTypeNodeExternalIP {
							...
							for _, address := range node.Status.Addresses {
								if address.Type == v1.NodeExternalIP || (address.Type == v1.NodeInternalIP && source.SuitableType(address.Address) == endpoint.RecordTypeAAAA) {
						...
						} else if endpointsType == source.EndpointsTypeHostIP || sc.publishHostIP {
					...
					for _, target := range targets {
						key := endpoint.EndpointKey{
							...
							RecordType: source.SuitableType(target),
						}
...
func (sc *serviceSource) endpoints(svc *v1.Service) []*endpoint.Endpoint {
	...
	// TODO: all allocated IPs ( v6 too)
	if svc.Spec.ClusterIP != "" {
		endpoints = append(endpoints, sc.generateEndpoints(svc, svc.Name + "." +
			svc.Namespace + sc.InternalDomain, nil, "", true)...)
	}
...
func (sc *serviceSource) generateEndpoints(svc *v1.Service, hostname string, providerSpecific endpoint.ProviderSpecific, setIdentifier string, useClusterIP bool) (endpoints []*endpoint.Endpoint) {
	...
	ttl := source.GetTTLFromAnnotations(svc.Annotations, resource)

	targets := source.GetTargetsFromTargetAnnotation(svc.Annotations)
	...
	endpoints = append(endpoints, source.EndpointsForHostname(hostname, targets, ttl, providerSpecific, setIdentifier, resource)...)
...
func (sc *serviceSource) extractNodePortTargets(svc *v1.Service) (endpoint.Targets, error) {
	...
	for _, node := range nodes {
		for _, address := range node.Status.Addresses {
			...
			case v1.NodeInternalIP:
				...
				if source.SuitableType(address.Address) == endpoint.RecordTypeAAAA {
	...
	access := source.GetAccessFromAnnotations(svc.Annotations)
	if access == "public" {
...
func (sc *serviceSource) AddEventHandler(ctx context.Context, handler func()) {
	...
	sc.serviceInformer.Informer().AddEventHandler(source.EventHandlerFunc(handler))
}
